...
public class RayTracerBasic extends RayTracerBase {
    ...
    private Color calcLocalEffects(GeoPoint geoPoint, Ray ray,Double3 k) {
        ...
        for (LightSource lightSource : scene.getLights()) {
            ...
            if (nl * nv > 0) { // sign(nl) == sing(nv)
               Double3 ktr= calculateHitColorPercentage(lightSource, geoPoint, n, l, nl, nv);//transparency(geoPoint,l,n,lightSource,nv);
    ...
    /**
     * Calculates the hit color percentage, which represents the percentage of rays that hit the object
     * out of all the rays generated by the points of the light source.
     *
     * @param lightSource The light source.
     * @param intersectionPoint The intersection point with the geometry.
     ...
     * @param dotProductNL The dot product between the normal vector and the light direction vector.
     * @param dotProductNV The dot product between the normal vector and the view direction vector.
     * @return The hit color percentage as a Double3 object.
     */
    private Double3 calculateHitColorPercentage(LightSource lightSource, GeoPoint intersectionPoint, Vector normal,
                                                Vector lightDirection, double dotProductNL, double dotProductNV) {
        Double3 averageColor = Double3.ZERO;
        Point[] samplePoints = lightSource.getPoints(numberOfPoints, intersectionPoint.point);

        if (samplePoints == null)
            return transparency(intersectionPoint, lightDirection, normal, lightSource, dotProductNL);

        for (Point samplePoint : samplePoints) {
            Vector rayDirection = intersectionPoint.point.subtract(samplePoint).normalize();
            averageColor = averageColor.add(
                    transparency(intersectionPoint, rayDirection, normal, lightSource, dotProductNL)
                            .reduce(samplePoints.length)
            );
        }
